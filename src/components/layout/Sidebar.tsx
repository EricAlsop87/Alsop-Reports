  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 px-3 space-y-4 mt-4 overflow-y-auto">
        ...
        <div className="px-3 mb-2 space-y-1">
          ...
          >
            <Settings className={cn(
              "shrink-0 transition-colors w-4 h-4",
              pathname.startsWith('/settings') ? "text-blue-600" : "text-slate-500 group-hover:text-slate-700 dark:text-slate-400 dark:group-hover:text-slate-300"
            )} />
      ...
      >
        ...
        <nav className="flex-1 px-2 space-y-4 mt-4 overflow-y-auto">
          {dynamicNavGroups.map((group) => {
            const filteredItems = group.items.filter(item => {
              if (item.pageKey === 'agent_portal' && !isManagerOrAdmin) return false
              if (item.pageKey === 'heatmap') return currentAgent?.role === 'admin'
            ...
            return (
              <div key={group.label} className="space-y-1">
                {isExpanded ? (
                  ...
                  <div className="h-px w-6 bg-slate-200 dark:bg-slate-700 mx-auto my-2 rounded-full" />
                )}
                {filteredItems.map((item) => {
                  const isActive = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href))
                  return (
